...
* [ ] O que é um **alfabeto `Σ`**; **É um conjunto finito de símbolos. Exemplo: `Σ = {a, b}`.**
* [ ] O que é uma **cadeia**; **É uma sequência de símbolos pertencentes a um alfabeto. Exemplo: `ab`.**
* [ ] O que significa **`ε`**; **É a palavra/cadeia vazia, ou seja, uma cadeia que não possui nenhum símbolo.**
* [ ] Por que **`|ε| = 0`**; **Porque `ε` não possui nenhum símbolo, então seu comprimento é zero.**
* [ ] O que é um **prefixo**; **É uma parte da palavra que começa no início. Exemplo: os prefixos de `ab` são `{ε, a, ab}`.**
* [ ] O que é um **sufixo**; **É uma parte da palavra que termina no final. Exemplo: os sufixos de `ab` são `{ε, b, ab}`.**
* [ ] O que significa **`Σ*`**; **É o conjunto de todas as cadeias finitas que podem ser formadas utilizando os símbolos de `Σ`, incluindo `ε`.**
* [ ] Se `Σ*` possui limite de tamanho; **Não. Não existe limite máximo para o tamanho das palavras, mas cada cadeia individual possui tamanho finito.**
* [ ] O que é uma **linguagem formal `L`**; **É um conjunto de palavras/cadeias construídas a partir de um alfabeto.**
* [ ] O que significa **`L ⊆ Σ*`**; **Significa que `L` é um subconjunto de `Σ*`, ou seja, as palavras de `L` fazem parte do conjunto de cadeias possíveis de `Σ*`.**
* [ ] O que é uma **gramática formal**; **É um conjunto de regras utilizado para gerar palavras/cadeias. Geralmente é representada por `G = (N, Σ, P, S)`.**
* [ ] O que são **terminais e não terminais**; **Terminais são os símbolos que formam as palavras finais da linguagem. Não terminais são símbolos usados durante a geração/derivação das palavras.**
* [ ] O que é uma **regra de produção**; **É uma regra que define como um símbolo pode gerar ou ser substituído por outros símbolos. Exemplo: `S → aS | ε`.**
* [ ] Como ler **`S → aS | ε`**; **Lê-se: `S` produz ou gera `aS` ou `ε`. O símbolo `|` significa “OU”.**
* [ ] Como gerar palavras usando uma gramática; **Começamos pelo símbolo inicial e aplicamos as regras de produção até chegar a uma palavra formada apenas por símbolos terminais. Exemplo: `S → aS → aaS → aaε → aa`.**
